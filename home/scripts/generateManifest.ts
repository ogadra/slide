...
// Slidev itself parses the headmatter with this same package, so a value is
// read here exactly the way the deck reads it.
...
// A space followed by `#` starts a YAML comment, so an event name like
// `埼京.dev #3【俺の考えた最強の◯◯】` loses everything from the `#` on. The rest is
// still a non-empty string, so nothing downstream would notice.
...
// Only the leading block is the headmatter. Every later `---` is a slide
// boundary, and some of those carry a per-slide frontmatter of their own.
...
const output = `// Generated by home/scripts/generateManifest.ts from the slides.md
// headmatter of every deck. Do not edit.
